...
def user( db ):
    ...
    user.security_answer_check( 'test_answer1' )
    db.session.add( user )
    db.session.commit()
    return user
...
def user_with_profile( db ):
    ...
    user.security_answer_check( 'test_answer3' )
    profile = Profile( username='username3',
...
def login( client, email, password, remember ):
    return client.post( '/signup/login/',
                        data=dict(
                            email=email,
                            password=password,
                            remember=remember
...
def logout( client ):
    return client.get( '/signup/logout/', follow_redirects=True )
